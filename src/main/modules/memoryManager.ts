...
const MEMORY_FILE_PATH = path.join(app.getPath('userData'), "memories.json");
...
export class MemoryManager {
  ...
  public static searchMemories(userId: string, query: string, category?: string): MemoryItem[] {
    const all = this.getMemories(userId);
    let results = all;
    if (category) {
    ...
    if (query) {
      ...
      results = results.filter(m => m.fact.toLowerCase().includes(q) || (m.notes && m.notes.toLowerCase().includes(q)));
    }
    return results;
  }
  ...
  public static formatIdentityMemoriesForPrompt(userId: string): string {
    ...
    if (identityMemories.length === 0) return "(No identity memories saved yet.)";
    return identityMemories.map((m) => `- [ID: ${m.id}] ${m.fact}`).join("\n");
  }

  public static formatEssentialMemoriesForPrompt(userId: string): string {
    const memories = this.getMemories(userId);
    if (memories.length === 0) return "(No prior memories saved yet.)";
    ...
    const essentials = memories.filter((m) => {
      ...
      if (factLower.includes("language") || factLower.includes("speak") || factLower.includes("hindi") || factLower.includes("english") || factLower.includes("bilingual") || factLower.includes("tone") || factLower.includes("style")) {
        return true;
      }
      return false;
    });

    if (essentials.length === 0) return "(No identity, pinned, or language memories saved yet.)";

    return essentials.map((m) => `- [ID: ${m.id}] [Category: ${m.category}] [Importance: ${m.importance || "normal"}] ${m.fact}${m.pinned ? " (PINNED)" : ""}`).join("\n");
  }

  public static formatMemoriesForPrompt(userId: string): string {
    const memories = this.getMemories(userId);
    if (memories.length === 0) return "(No prior long-term memories saved yet.)";

    const categories: Record<string, string[]> = {
      identity: [], preference: [], goal: [], project: [], relationship: [], emotional: [], behavioral: []
    };
    ...
    const categoryLabels: Record<string, string> = {
      identity: "IDENTITY", preference: "PREFERENCES", goal: "GOALS & ASPIRATIONS", project: "ONGOING PROJECTS", relationship: "RELATIONSHIPS", emotional: "EMOTIONALLY SIGNIFICANT EVENTS", behavioral: "BEHAVIORAL HABITS"
    };
...
export function setupMemoryHandlers() {
  ipcMain.handle('memory:getMemories', (_, userId) => MemoryManager.getMemories(userId));
  ipcMain.handle('memory:saveMemory', (_, userId, fact, category, importance, archived, pinned, notes) => 
    MemoryManager.saveMemory(userId, fact, category, importance, archived, pinned, notes));
  ipcMain.handle('memory:updateMemory', (_, userId, id, fact, category, importance, archived, pinned, notes) =>
    MemoryManager.updateMemory(userId, id, fact, category, importance, archived, pinned, notes));
  ipcMain.handle('memory:forgetMemory', (_, userId, id) => MemoryManager.forgetMemory(userId, id));
  ipcMain.handle('memory:isMemoryPaused', (_, userId) => MemoryManager.isMemoryPaused(userId));
  ipcMain.handle('memory:setMemoryPaused', (_, userId, paused) => MemoryManager.setMemoryPaused(userId, paused));
  ipcMain.handle('memory:getFormattedMemoriesForPrompt', (_, userId, essentialOnly) => 
    essentialOnly ? MemoryManager.formatEssentialMemoriesForPrompt(userId) : MemoryManager.formatMemoriesForPrompt(userId));
}
